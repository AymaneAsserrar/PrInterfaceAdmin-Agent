...
class MonitorTaskFake(MonitorTask):
    ...
    Provides deterministic values for testing.
    """
    def __init__(self): 
        # Initialize with proper types for all attributes
        self.interval = 0
        # CPU percentages should be floats, not strings
        self.cpu_percent = [10.0, 12.0]
        self.num_cores = 2  # Match the length of cpu_percent
        # RAM values in MB
...
def test_get_cpu_usage():
    ...
    try:
        ...
        assert cpu_usage == expected_cpu_data, "CPU usage data doesn't match expected values"
